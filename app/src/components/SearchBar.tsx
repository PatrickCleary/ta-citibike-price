...
export function SearchBar({ stations, mapCenter, onSelect }: SearchBarProps) {
  ...
  const searchStations = useMemo(() => createStationSearcher(stations), [stations]);
  ...
  const runSearch = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const addressResults = await searchAddresses(q, mapCenter, controller.signal);
  ...
  return (
      <div className="absolute top-3 right-3">
        ...
        <input
          ...
          className="w-sm rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-9
        ...
        {open && !loading && query.length >= 3 && suggestions.length === 0 && (
          <div className="absolute z-20 mt-2 w-full rounded-2xl border border-slate-100 bg-white
